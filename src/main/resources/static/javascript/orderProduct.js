...
function orderProductInsert() {
	...
	// axios 요청
	let url="/api/cart/orderProductInsertP";
...
function orderProductUpdate() {
	...
	// axios 요청
	let url="/api/cart/orderProductUpdateP";
...
function opDelete(order_key) {
	let url = "/api/cart/opdelete/" + order_key;
...
function orderProduct(a) {
	let url = "/api/cart/orderProduct" + a;
	axios.get(url
	).then(response => {
		document.getElementById('resultArea2').innerHTML = response.data;
	}).catch(err => {
